...
from server import NotFoundError
...
invalid_categories_id = [
    ...
    (1 + max(list_categories, key=category_id_pred).reference, choice(list_categories).account),
]
invalid_accounts = [
    (choice(list_categories).reference, -1),
    (choice(list_categories).reference, max(list_categories, key=account_pred).account + 1),
]
...
class FakeCategories(CategoryRepository, Mock):
    ...
    def from_id(self, identity: int) -> Category:
        return copy(self.contain(lambda category: category.reference == identity)[0])
    ...
    def insert(self, category: Category) -> int:
        newest_id = 1 + max(self.categories, key=lambda _category: _category.reference, default=-1).identity
    ...
    def save(self, category: Category) -> None:
        category_in_system = self.contain(
            lambda _category: _category.reference == category.reference)[0]
...
@pytest.fixture
def last_id():
    return max(list_categories, key=lambda category: category.reference).reference
...
class TestCategoryService:
    ...
    @pytest.mark.parametrize(
        ...
        [(category.reference, category.account, category) for category in list_categories])
    ...
    @pytest.mark.parametrize(
        ...
        [(category.reference, category.account, category.name) for category in list_categories] +
        [(category.reference, category.account, 'new_category-{i}')
    ...
    @pytest.mark.parametrize(
        'identity, account_id',
        [(category.reference, category.account) for category in list_categories])
