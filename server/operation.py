...
def update_task_category(db: SQLiteDB, id_task: int, new_id_category: int):
    db.is_exist_task(id_task)
    db.is_exist_category(new_id_category)
...
def update_category_name(db: SQLiteDB, destination_id: int, source: str):
    db.is_exist_category(destination_id)
    if destination_id == 1:
...
@error_handler
def delete_task(db: SQLiteDB, id_task: int, id_file: int):
    answer = delete_file(id_task, id_file)
