...
from server.account.business import (
    AccountService, AccountInputData,
    AccountRepository, Account, AccountStatus, Role,
)
from server.account.business.entity import PasswordHash, AccessError, UnconfirmedEmailError
from server.error.errors import NotFoundError, LoginError, DataUniqueError

list_users = [
    Account.Generator.owner(0, AccountStatus.CONFIRMED),
    Account.Generator.admin(1, AccountStatus.CONFIRMED),
    Account.Generator.admin(2, AccountStatus.REFUSED),
    Account.Generator.user(3, AccountStatus.CONFIRMED),
    Account.Generator.user(4, AccountStatus.NOT_CONFIRMED),
    Account.Generator.user(5, AccountStatus.REFUSED),
]
id_users = [user.id for user in list_users]
users_by_uuid = {uuid.uuid4(): user for user in list_users}
invalid_ids = [-1, max(id_users) + 1]
confirmed_users = list(filter(lambda user: user.email_status == AccountStatus.CONFIRMED, list_users))
unconfirmed_users = list(filter(lambda user: user.email_status != AccountStatus.CONFIRMED, list_users))

admin_ids = [
    user.id for user in list_users if
    user.role == Role.ADMIN and user.email_status == AccountStatus.CONFIRMED or user.role == Role.OWNER
]

not_admin_ids = [
    user.id for user in list_users if
    not (user.role == Role.ADMIN and user.email_status == AccountStatus.CONFIRMED or user.role == Role.OWNER)
]


class FakeUsers(AccountRepository, Mock):
    ...
    def create(self, user):
        ...
        user.identity = self.generate_id()
    ...
    def get(self, pred: Callable) -> Account:
        result = list(filter(pred, self.users))
        if len(result) == 0:
            raise NotFoundError()
        return result[0]

    def get_by_id(self, user_id: int) -> Account:
...
class TestUserService:
    ...
    def setup_method(self, method):
        self.service = AccountService(FakeUsers)

    @pytest.mark.parametrize('user_id, account', [(user.id, user) for user in list_users])
    def test_get_account__found(self, user_id: int, user: Account):
    ...
    @pytest.mark.parametrize('account, email, password',
                             [(user, 'new_email-{account.id}@domen.com', user.name) for user in list_users] +
                             [(user, user.email, 'new-password') for user in list_users]
                             )
    def test_update_account__success(self, user: Account, email: str, password: str):
        assert self.service.update_account(user.id, AccountInputData(password, email=email)) is None
    ...
    def test_update_account__no_change(self, user_id: int, email: str, password: str):
        assert self.service.update_account(user_id, AccountInputData(password, email=email)) is None
        assert self.service.get_account(user_id).email == email

    @pytest.mark.parametrize('user_id', invalid_ids)
    def test_update_account__not_found_error(self, user_id):
        with pytest.raises(NotFoundError):
            self.service.update_account(user_id, AccountInputData(password='unique', email='unique'))

    def test_update_account__data_unique_error(self):
        with pytest.raises(DataUniqueError):
            self.service.update_account(1, AccountInputData(password='unique', email=list_users[-1].email))
    ...
    def test_register__success(self):
        self.service.register(AccountInputData(name='unique', email='unique@mail.com', password='password'))
        with pytest.raises(UnconfirmedEmailError):
            self.service.login_by_name(AccountInputData(name='unique', password='password'))
    ...
    def test_register__data_unique_error(self, name: str, email: str):
        with pytest.raises(DataUniqueError):
            self.service.register(AccountInputData(name=name, email='unique@domen.ru', password="password"))
        with pytest.raises(DataUniqueError):
            self.service.register(AccountInputData(name='unique', email=email, password="password"))
        with pytest.raises(DataUniqueError):
            self.service.register(AccountInputData(name=name, email=email, password="password"))
    ...
    def test_login_by_name__success(self, name: str, password: str, user_id: int):
        assert self.service.login_by_name(AccountInputData(password, name=name)) == user_id
    ...
    def test_login_by_email__success(self, email: str, password: str, user_id: int):
        assert self.service.login_by_email(AccountInputData(password, email=email)) == user_id
    ...
    def test_login_by_name__unconfirmed_email_error(self, name: str, password: str):
        with pytest.raises(UnconfirmedEmailError):
            self.service.login_by_name(AccountInputData(password, name=name))
    ...
    def test_login_by_email__unconfirmed_email_error(self, email: str, password: str):
        with pytest.raises(UnconfirmedEmailError):
            self.service.login_by_email(AccountInputData(password, email=email))
    ...
    def test_login_by_name__login_error(self, name: str, password: str):
        with pytest.raises(LoginError):
            self.service.login_by_name(AccountInputData(name, password))
    ...
    def test_login_by_email__login_error(self, email: str, password: str):
        with pytest.raises(LoginError):
            self.service.login_by_email(AccountInputData(email, password))

    @pytest.mark.parametrize('token, account', users_by_uuid.items())
